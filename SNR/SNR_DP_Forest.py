...
class SNR_DP_Forest:  
    def __init__(self, 
        ...
        ''' Some initialization '''
        attribute_values = self.get_domains(training_data)  # 属性域
        class_values = [str(y) for y in list(set([x[0] for x in training_data]))]  #class域
        attribute_indexes = [int(k) for k,v in attribute_values.items()]
        dataset_size = len(training_data)  # 训练集长度
        valid_attribute_sizes = [[int(k),len(v)] for k,v in attribute_values.items()] #返回[属性,属性值个数]
        num_trees = len(attribute_indexes)
        average_domain = np.mean( [x[1] for x in valid_attribute_sizes] )  # 属性值的平均个数
    ...
    def evaluate_accuracy_with_voting(self, records, class_index=0):
        ...
        for rec in records:
            ...
            predicted_labels.append(float(best[0]))
...
if __name__ == '__main__':
    train_data = pd.read_csv('../dataset/train_pima.csv', sep=',', header=0)
    test_data = pd.read_csv('../dataset/test_pima.csv', sep=',', header=0)
    train_data = train_data.values
    test_data = test_data.values
    es = [1, 0.8, 0.6, 0.4, 0.2]
    file_name = 'data_column.xlsx'
    ...
    for epsilon in es:
        accu = []
        for j in range(10):  # 执行10次求平均
            our_diff_priv_forest = SNR_DP_Forest(train_data, epsilon)
            ...
            accuracy, leafs_not_most_confident, votes_requiring_average = our_diff_priv_forest.evaluate_accuracy_with_voting(
                test_data, class_index=0)  # confidence count includes all trees (before voting)
            print("accuracy = {}".format(accuracy))
            accu.append(accuracy)
        row_number = 1
        for i in accu:
            sheet.cell(row=row_number, column=column_number, value=i)
            row_number += 1
            print("平均值为：", i)
        column_number += 1
    # 保存工作簿到文件
