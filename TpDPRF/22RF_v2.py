...
class Random_Forest(object):
    ...
    def __init__(self, n_estimators: int, estimator, n_jobs=-1, rate=0.7):
        ...
        if n_jobs == -1:
            self.n_jobs = multiprocessing.cpu_count()  # 获得cpu核心数
...
def kfold_function(x, y, cols, num, k , epsilon, n_estimators, max_depth = 8, function="self_rf", average="macro", n_jobs = -1):
    '''
    ...
    for i in range(num):
        ...
        for train_index, test_index in kf.split(x, y):
            ...
            # if function == "self_rf":
            estimator = Tree(cols, privacy_e=epsilon, max_depth=max_depth)  # 基分类器：多变量决策树
            seps += epsilon
...
def test_processed_hungarian_kfold(data, num, k, epsilon, h):
    from sklearn.model_selection import KFold
    from sklearn.utils import shuffle
    ...
    mdlp = MDLPDiscretizer(
        features=[], return_intervals=False, return_df=True)
    data_x = mdlp.fit_transform(data_x, data_y)
    print(data_x)
    data_x, data_y = shuffle(data_x, data_y)  # 打乱数据
    x, y = data_x.values, data_y.values # pandas DataFrame转numpy array

    acc = kfold_function(x, y, cols, num=num, k=k, epsilon=epsilon/k, function="self_rf", max_depth=h, average=None, n_estimators=k)
    print(acc)
    return acc
...
if __name__ == "__main__":
    ...
    k = 5
    num = 1
    # data = pd.read_csv("Input/mushuroom.data", sep=",", header=None)
    ...
    column_number = 1  # 列号，例如第1列
    for epsilon in es:
        data = pd.read_csv("Input/diabetes.csv", sep=",", header=None)
        h = int((data.shape[1] - 1) / 2)  # 高度
        acc = test_processed_hungarian_kfold(data, num, k, epsilon, h)
