...
class Tree(object):
    ...
    def fit(self, x, y):
        ...
        '''
        # current_depth = 0
        self.tree_ = self._build_tree(x, y, current_depth=1)  # 这一点测试一下
        print("sum_eps", self.sum_eps)
        return self.tree_
    ...
    def _expmechanism(self,score, m, epsilon, sensitivity):  #实现差分隐私指数机制
        exponents_list = []
    ...
    def _build_tree(self, x, y, current_depth=1, key=None):
        ...
        e_current = ((self.privacy_e) / s) * (1 / (self.max_depth - current_depth + 1))  # 计算每层的隐私预算
        # print('e_curent:', e_current)
        if self.high == current_depth:
            self.high += 1
            self.sum_eps += e_current
        ...
        max_gini_threshold, max_gini_w, max_gini_feas = self._find_best_split(
            x, y, feas, number=number, delete_rate=None, e_current=e_current)  #因为指数机制和拉普拉斯机制不会一起使用，所以没有进一步划分隐私预算
        left_array, right_array, left_list, right_list = self._split_train_data_by_value(
...
def test_processed_hungarian():
    ...
    data = pd.read_csv("Input/adult.csv", sep=",", header=None)
    ...
    data_x = data.iloc[:, 0:-1]  # 包含了 data DataFrame 中的所有行和除最后一列之外的所有列，因此它包含了特征数据。
    data_y = data.iloc[:, -1]   # 包含了 data DataFrame 中的所有行和最后一列，因此它包含了目标数据。
    cols = data_x.columns.tolist()  # 特征名称转list
    print(data_x)
   
    # 离散化
    mdlp = MDLPDiscretizer(
        features=[0, 2, 4, 10, 11, 12], return_intervals=False, return_df=True)
    data_x = mdlp.fit_transform(data_x, data_y)
    print(data_x)
    x_train, x_test, y_train, y_test = train_test_split(
        data_x, data_y, test_size=0.2)  #8 2 分训练集和测试集
    ...
    tree = Tree(feas=cols, delete_rate=0.1, eps=0.02,
                sample_rate=0.95, max_depth=10, name=None, criterion="entropy")  #在此设置各种参数
    tree.fit(x_train, y_train)
    predict_y = tree.predict(x_test)
    print("tree\n{0}\n".format(tree.tree_))
